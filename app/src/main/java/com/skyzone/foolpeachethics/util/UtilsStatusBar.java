...
/**
 * Created by userdev1 on 2018/7/23.
 */
...
public class UtilsStatusBar {
    ...
    /**
     * @param activity
     * @param useThemestatusBarColor   是否要状态栏的颜色，不设置则为透明色(背景颜色：true是非透明色 false是透明色)
     * @param withoutUseStatusBarColor 是否不需要使用状态栏为暗色调 (字体颜色：true是白色 false是暗色)
     */
...
//                attributes.flags |= flagTranslucentNavigation;
    ...
    public static void setStatusBarColor(Activity activity, int color) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP && color != 0) {
            try {
                activity.getWindow().setStatusBarColor(MyApp.mContext.getResources().getColor(color));
            }catch (Exception e){
                try {
    ...
    public static void setStatusBarLight(Activity activity, boolean light) {
        ...
        try {
            ...
            darkModeFlag = field.getInt(layoutParams);
            Method extraFlagField = clazz.getMethod("setExtraFlags", int.class, int.class);
            extraFlagField.invoke(activity.getWindow(), light ? darkModeFlag : 0, darkModeFlag);
        } catch (Exception e) {
        }
    }
